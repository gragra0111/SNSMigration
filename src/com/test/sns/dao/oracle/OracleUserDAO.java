...
public class OracleUserDAO {
	...
	public String setUserId() {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT CASE TO_NUMBER(TO_CHAR(SYSDATE,'YYYY')) - TO_NUMBER(TO_CHAR(MAX(create_dt),'YYYY')) WHEN 0 THEN 'U' || TO_CHAR(SYSDATE,'YYYY') || LPAD(TO_NUMBER(SUBSTR(MAX(user_id),10))+1,9,'0') ELSE 'U' || TO_CHAR(SYSDATE,'YYYY') || '000000001' END AS userId FROM temp_tb_user");
			rs = ps.executeQuery();
	...
	public void insert(OracleUserDTO tb_userDto) {
		...
		try {
			conn = dataSource.getConnection();
			ps = conn.prepareStatement("INSERT INTO temp_tb_user (user_id, user_nm, lgn_id, emp_no, pst, mail, cp_no, offc_phon_no, use_yn, create_dt, temp_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, SYSTIMESTAMP, ?)");
