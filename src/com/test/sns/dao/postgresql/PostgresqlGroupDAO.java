...
public class PostgresqlGroupDAO {
	...
	public String setGroupId() {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT CASE TO_NUMBER(TO_CHAR(NOW(),'YYYY'), '9999') - TO_NUMBER(TO_CHAR(MAX(create_dt),'YYYY'), '9999') "
				+ "       WHEN 0 THEN 'G' || TO_CHAR(NOW(),'YYYY') || LPAD(TRIM(TO_CHAR(TO_NUMBER(SUBSTR(MAX(grp_id),6), '999999999')+1, '999999999')),9,'0')"
				+ "       ELSE 'G' || TO_CHAR(NOW(),'YYYY') || '000000000' END AS grpId FROM temp_tb_group"
			);
			rs = ps.executeQuery();
	...
	public void insert(OracleGroupDTO tb_groupDto) {
		...
		try {
			conn = dataSource.getConnection();
			ps = conn.prepareStatement("INSERT INTO temp_tb_group (grp_id, grp_nm, grp_expn, create_id, create_dt, temp_grp_id) VALUES (?, ?, ?, ?, LOCALTIMESTAMP, ?)");
